...
func String2BigIntTuple(s string) (big.Int, big.Int) {
	bx, _ := hex.DecodeString(s[:64]) //decode string takes a hexadecimal string and returns a byte slice
	by,_ := hex.DecodeString(s[64:])

	var bix big.Int
	var biy big.Int

	_ = bix.SetBytes(bx) //takes the hexadecimal byte slice and returns a big.Int
	_ = biy.SetBytes(by)
